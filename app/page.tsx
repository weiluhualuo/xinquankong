      ...
      <div className="min-h-screen bg-slate-50 text-slate-900">
        <section className="relative mx-auto max-w-6xl px-4 py-12 md:py-20">
          <div className="relative max-w-4xl">
            ...
            <h1 className="mt-4 text-3xl font-black tracking-tight sm:text-4xl md:text-5xl lg:text-6xl">{homepageContent.heroTitle}</h1>
            <p className="mt-4 max-w-2xl text-base leading-7 text-slate-600 md:text-lg md:leading-8">{homepageContent.heroDescription}</p>
            <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:gap-4">
              <Link href="/publish" className="rounded-xl border border-slate-900 bg-slate-900 px-6 py-3 text-center text-sm font-semibold text-white">写一篇帖子</Link>
          ...
          <div className="mx-auto max-w-6xl px-0 py-0">
            <div className="bg-gradient-to-br from-white via-slate-50 to-[var(--accent)]">
              <div className="grid grid-cols-1 lg:grid-cols-[220px_1fr_300px]">
                  <section className="rounded-none border-b border-slate-200 bg-white p-0 shadow-none lg:border-b-0 lg:border-r">
                    ...
                    <div className="px-5 pb-5">
                      {boards.map((board, index) => (
                        <Link key={board.id} href={`/board/${board.slug}`} className={`block py-4 transition-all hover:bg-slate-50 ${index > 0 ? "border-t border-slate-200" : ""}`}>
                  ...
                  <section className="rounded-none bg-slate-50 shadow-none">
                    ...
                    <div className="mt-5 space-y-3 px-5 pb-5">
                      {hot.items.slice(0, 5).map((post, index) => (
                        <Link key={post.id} href={`/post/${post.id}`} className="block rounded-2xl border border-slate-200 bg-white p-4 transition hover:-translate-y-0.5 hover:shadow-md hover:border-slate-300">

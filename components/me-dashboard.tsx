...
import { ApiError, getMe, getMyActivity } from "../lib/api";
import { MyActivity, UserProfile } from "../lib/types";
...
export function MeDashboard() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        const [user, activityData] = await Promise.all([getMe(), getMyActivity(10)]);
        setMe(user);
  ...
  if (loading) {
    return (
      <div className="min-h-[calc(100vh-3.5rem)] bg-[#f8fafc] py-12 md:py-20">
        <div className="mx-auto max-w-4xl px-4">
          <div className="animate-pulse space-y-6 rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
            <div className="h-8 w-48 rounded bg-slate-100"></div>
            <div className="h-20 rounded bg-slate-50"></div>
            <div className="h-40 rounded bg-slate-50"></div>
          </div>
        </div>
      </div>
    );
